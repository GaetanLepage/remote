...
class Server:
    ...
    def client(self) -> SSHClient:
        if self.__client is None:
            ...
            with monit.section(f'Connecting to {self.conf.hostname}'):
                ...
                self.__client.connect(hostname='alya.inrialpes.fr',
                                      username='galepage',
                                      port=22,
                                      sock=paramiko.ProxyCommand(
                                          command_line='ssh -W %h:%p galepage@bastion.inrialpes.fr'),
                                      banner_timeout=20)
                                      # sock=paramiko.ProxyCommand(command_line='ssh -q -W %h:%p "\
                                      #     "galepage@bastion.inrialpes.fr'),
    ...
    def setup(self, *, ui_mode: UIMode = UIMode.dots):
        ...
        with monit.section(f"Setup server {self.conf.name}"):
            logger.log()
            if self.remote_exec.eval(f'test -d {self.project_name}',
                                     log_dir=None).exit_code != 0:
                self.remote_exec.eval(f'mkdir {self.project_name}',
                                      log_dir=None)
            if self.remote_exec.eval(f'test -d {self.remote_scripts_path}',
                                     log_dir=None).exit_code != 0:
                self.remote_exec.eval(f'mkdir {self.remote_scripts_path}',
                                      log_dir=None)
            ...
            if self.copy_and_run_script(script=script,
    ...
    def copy_script(self,
        ...
        scripts_path: Path = Configs.get().project_scripts_folder
        ...
        os.chmod(str(script_file),
                 stat.S_IRWXU | stat.S_IRWXG)
        scp: SCPClient = SCPClient(self.client.get_transport())
        remote_path: str = f'{self.remote_scripts_path}/{script_name}'
    ...
    def copy_and_run_script(self,
                            ...
                            *,
                            ...
                            is_background=False,
                            is_eval=False) -> EvalResult:
        remote_path: str = self.copy_script(script, script_name)
        logger_name: str = script_name.replace('.', '__')
        log_dir = self._get_log_folder(f'{logger_name}_{self.conf.name}')

        return self.run_script(remote_path,
                               log_dir=log_dir,
                               ui_mode=ui_mode,
    ...
    def rsync(self, *, ui_mode: UIMode = UIMode.dots):
        with monit.section(f"RSync {self.conf.name}"):
            ...
            # destination
            # TODO: make it compatible with inria
            rsync_cmd += [f'{self.conf.username}@{self.conf.hostname}:~/{self.project_name}/']
    ...
    def update_packages(self, *, ui_mode: UIMode = UIMode.dots):
        ...
        with monit.section(f"Update python packages {self.conf.name}"):
            ...
            script = self.template_script(script_name='update.sh',
                                          replace={'has_pipfile': str(pipfile.exists()),
                                                   'has_requirements': str(requirements.exists())})

            if self.copy_and_run_script(script, 'update.sh',
                                        ui_mode=ui_mode).exit_code != 0:
                raise RemoteError("Failed to update pckages")
